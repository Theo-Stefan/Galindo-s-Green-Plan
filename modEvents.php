...
<head>
  ...
  <body>
    <?php
      ...
      $sql = "SELECT id, title, description, date, url, image, priority FROM Events;";
    ...
    <div class="main">
        <div class="list">
          ...
          <?php   // Load the news dynimically
            $counter = 0;
            $pointer = array();   // To know whick event has which info

            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
              while($row = $result->fetch_assoc()) {
                echo "<form action=\"modEvents.php\" method=\"post\">
           ...
           ?>
           <form action="modEvents.php" method="post">
        ...
        <div class="working-frame">
          ...
          <?php
                  ...
                  if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    if(isset($_POST["delete"])) {
                      ...
                      while($row = $result->fetch_assoc()){
                        if ($row["id"] == $_SESSION["newsId"]){
                          $sqlDelete = "DELETE FROM Events WHERE id=".$row["id"].";";
                    ...
                    if (isset($_POST["create"])){
                      ...
                      if ($flag){  // Everything is fine in the data that were given
                        $sqlInsert = "INSERT INTO Events (title, description, date, url, image, priority)
                    ...
                    if (isset($_POST["reload"]) || isset($_POST["workshop"])){
                      ...
                      echo "<form method=\"post\" action=\"moderator.php\">
                              <center><button type=\"submit\" name=\"workshop\"> Add News </button></center>
                            </form>";
                    }
                  ...
                  function mainFrameCreate($t, $des, $d, $url, $im, $pr){
                    echo "<form action=\"modEvents.php\" method=\"post\" enctype=\"multipart/form-data\">
                          <center>
                            <h2> Title </h2>
                    ...
                    while($row = $result->fetch_assoc()){   // If the same priority exists push the existent
                      if ($row["priority"] == $_POST["priority"]){  // one up (until it is okay)
                        ...
                        while ($chprio){    // finde the sweet spot to enter the new priority
                          ...
                          if (!$chprio){
                            $sqlUpdate = "UPDATE Events SET priority = $otherPriority WHERE id=$otherId;";
